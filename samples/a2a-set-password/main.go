...
// Command a2a-set-password demonstrates SetPassword, the A2A write-back that
// stores a new password for an account. The application authenticates with its
// client certificate and presents an API key that authorizes the account. The
// account's A2A registration must have bidirectional (write-back) access enabled,
// or the appliance rejects the call.
package main
...
func main() {
	...
	apiKeyFlag := flag.String("apikey", "", "the A2A API key that authorizes the account")
	newPasswordFlag := flag.String("new-password", "", "the new password to store")
	flag.Parse()

	if *appliance == "" || *certFile == "" || *apiKeyFlag == "" || *newPasswordFlag == "" {
		log.Fatal("-appliance, -cert, -apikey, and -new-password are required")
	}
	apiKey := safeguard.NewSecretString(*apiKeyFlag)
	defer apiKey.Zero()
	newPassword := safeguard.NewSecretString(*newPasswordFlag)
	...
	if err := a2a.SetPassword(ctx, apiKey, newPassword); err != nil {
