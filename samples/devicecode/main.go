...
// Command devicecode demonstrates a device authorization grant login. The SDK
// prints a verification URL and user code; the user completes authentication in
// any browser (including on another device) and the SDK polls until it succeeds.
// It lives in the devicecode add-on package.
package main
...
func main() {
	...
	prompt := func(_ context.Context, p devicecode.Prompt) error {
		...
		fmt.Printf("Waiting for you to finish (expires in %s)...\n", p.ExpiresIn)
		return nil
	}

	ctx := context.Background()
	client, err := devicecode.Connect(ctx, *appliance, prompt, devicecode.WithConnectionOptions(connOpts...))
	if err != nil {
