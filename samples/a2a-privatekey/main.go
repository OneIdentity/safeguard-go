...
// Command a2a-privatekey demonstrates Application-to-Application (A2A) SSH
// private-key retrieval. The application authenticates with its client
// certificate over mutual TLS and presents an API key that authorizes retrieval
// of one account's SSH private key.
package main
...
func main() {
	...
	format := flag.String("format", string(safeguard.KeyFormatOpenSSH), "key format: OpenSsh, Ssh2, or Putty")
	flag.Parse()
	...
	privateKey, err := a2a.RetrievePrivateKey(ctx, apiKey, safeguard.KeyFormat(*format))
	if err != nil {
		log.Fatalf("retrieve private key: %v", err)
	}
	defer privateKey.Zero()

	// A real application would use the key, not print it.
	fmt.Printf("Retrieved private key:\n%s\n", privateKey.ExposeString())
}
