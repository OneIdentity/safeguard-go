...
// Command a2a-password demonstrates Application-to-Application (A2A) password
...
func main() {
	...
	if err != nil {
		log.Fatalf("retrieve password: %v", err)
	}
	defer password.Zero()

	// A real application would use the credential, not print it.
	fmt.Printf("Retrieved password: %s\n", password.ExposeString())
}
