...
// Command pkce demonstrates a PKCE non-interactive ("headless") login. This is
// the recommended flow for test automation because it does not depend on the
// Resource Owner Grant, which appliances commonly disable.
package main
...
func main() {
	...
	remaining, err := client.TokenLifetimeRemaining(ctx)
	if err != nil {
		log.Fatalf("token lifetime: %v", err)
	}
	fmt.Printf("Token lifetime remaining: %s\n", remaining)
}
