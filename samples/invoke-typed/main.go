...
// Command invoke-typed demonstrates InvokeTyped, which decodes a successful JSON
// response directly into a Go value instead of returning the raw body. It signs
// in with the PKCE headless flow and reads the current user into a struct.
package main
...
func main() {
	...
	// InvokeTyped decodes the response body into the provided type. Only the
	// fields of interest need to be declared.
	type me struct {
		ID          int    `json:"Id"`
	...
	fmt.Printf("Signed in as %s (%s), id=%d\n", info.UserName, info.DisplayName, info.ID)
